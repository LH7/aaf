...
void LogAAFBufferStat(const wchar_t *filename, const AAF_buffer_t *AAFb)
{
    ...
    get_date(date_str);
    fprintf(file,
        "%s,rqs,%"PRId64",%"PRId64",%"PRId64",%"PRId64",%"PRId64",%"PRId64",%"PRId64",%"PRId64",%"PRId64",%"PRId64",%"PRId64",%"PRId64",%"PRId64",%"PRId64",%"PRId64"\n",
        date_str,
...
void LogAAFBufferHeaders(const wchar_t *filename)
{
    ...
    get_date(date_str);
    fprintf(file,
        "%s,hdr,request.PID,request.hFile,request.blockSize,request.alignSize,result.success,result.statusCode,stats.fileLength,stats.fileFragments,stats.allocOffset,stats.allocTime,stats.searchSkip,stats.searchTotal,stats.moveAttempts,stats.moveTime,stats.prefetchTime\n",
        date_str
    );
    fclose(file);
}
